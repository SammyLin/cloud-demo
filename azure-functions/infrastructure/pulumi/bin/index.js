...
const functionApp = new azure.web.WebApp(functionAppName, {
    ...
    siteConfig: {
        ...
        javaVersion: "17",
        alwaysOn: true, // Enable always on to support blob triggers
    },
});
...
if (enableCosmosDb && cosmosAccount) {
    ...
    serviceConnector = new azure.servicelinker.Linker("cosmosdb-connection", {
        resourceUri: pulumi.interpolate `/subscriptions/${subscription.then((sub) => sub.subscriptionId)}/resourceGroups/${rg.name}/providers/Microsoft.Web/sites/${functionApp.name}`,
        targetService: {
            type: "AzureResource",
            id: cosmosAccount.id,
        },
